...
functionality and can be tested in a future test campaign.~
    \medskip
    \end{minipage}
    &
    \\\hline
    ...
    \begin{minipage}[]{9cm}
    \smallskip
    The test campaign for the Camera Hardware Functional Re-verification was
seen as: FAIL.\\[2\baselineskip]Overall, the test campaign was
successful but considered a ``FAIL'' due to one failed step requiring a
software fix. For the complete test results, see \textbf{LSST Rotator
Acceptance Test Results} in
\url{https://docushare.lsstcorp.org/docushare/dsweb/View/Collection-8902/Document-35429}
.
    \medskip
    \end{minipage}
    &
...
Hardware Functional Re-verification test failed because we could not
change the software limits to verify the limit switches. The software
will be updated to fix this issue and it will be tested with the fix in
the next test campaign (See Recommended Improvements). The majority of
the Camera Rotator Software Functional Re-verification test steps
passed, but some were left unexecuted and will also be tested during the
next test campaign.\\[2\baselineskip]
...
Final comment:\\The test campaign for the Camera Hardware Functional Re-verification was
seen as: FAIL.\\[2\baselineskip]Overall, the test campaign was
successful but considered a ``FAIL'' due to one failed step requiring a
software fix. For the complete test results, see \textbf{LSST Rotator
Acceptance Test Results} in
